...
export const getCampaignList = async (
  ...
  // Vercel 환경에서 메인페이지(/ 또는 /main)에서는 무조건 목데이터 사용
  const isMainPage = window.location.pathname === '/' || window.location.pathname === '/main'
  if (isVercel && isMainPage) {
    console.log('Vercel + 메인페이지 - Mock 데이터 강제 사용')
